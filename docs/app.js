...
const relatedExclusionWords = ['登場人物一覧', '人物一覧', '一覧', '記念館', '施設', '会社', 'キャラクター一覧', 'エピソード一覧', '用語一覧', '関連項目'];
...
function route(name) {
  ...
  if (name === 'settings') {
    ...
    $('#searchDebugInput').checked = isSearchDebugEnabled();
  }
...
function emptyBuckets(query) {
  return {query, overview: [], 漫画: [], アニメ: [], ドラマ: [], 映画: [], 小説: [], similar: [], genres: new Set(), sources: [], discovery: [], exactCandidate: [], aliasCandidate: [], strongCandidate: [], fuzzyCandidate: [], relatedCandidate: []};
}
...
function sourceCounts(buckets) {
  return displayCandidates(buckets).reduce((counts, item) => {
...
function classifyCandidate(item, query) {
  ...
  const wikidataContains = source.includes('wikidata') && naturallyContainsQuery(item, query);
  const fuzzy = [item.title, ...aliases].some(value => normalizedSimilarity(value, query) >= 0.62);
  if (isRelatedExcluded(item)) return 'relatedCandidate';
  if (exact) return 'exactCandidate';
  if (aliasExact) return 'aliasCandidate';
  if ((close || wikidataContains) && !(isProbablyJapanese(query) && source.includes('anilist') && !aliasExact)) return 'strongCandidate';
  if (close || fuzzy) return 'fuzzyCandidate';
  return 'relatedCandidate';
}
function isRelatedExcluded(item) {
  const text = `${item.title || ''} ${item.description || ''}`;
  return relatedExclusionWords.some(word => text.includes(word));
}
function naturallyContainsQuery(item, query) {
  const nq = normalizedText(query);
  if (nq.length < 3) return false;
  return extractAliasesFromItem(item).some(value => {
    const normalized = normalizedText(value);
    return normalized !== nq && normalized.includes(nq);
  });
}
function mainCandidates(data) { return displayCandidates(data).filter(item => ['exactCandidate', 'aliasCandidate', 'strongCandidate'].includes(item.confidence || classifyCandidate(item, data.query))); }
...
function displayCandidates(data) { return uniqueBy([...allCandidates(data), ...(data.exactCandidate || []), ...(data.aliasCandidate || []), ...(data.strongCandidate || [])].map(normalizeItem), duplicateKey); }
...
function mediaSummary(data) { return judgementMedia.map(media => ({media, count: mainCandidates(data).filter(item => item.media === media).length})); }
function mediaDisplayItems(data, media) { return uniqueBy([...(data[media] || []), ...mainCandidates(data).filter(item => item.media === media)], duplicateKey); }
...
async function searchWikidata(terms, query) {
  ...
  for (const term of uniqueTerms(Array.isArray(terms) ? terms : [terms], 6)) {
    ...
    (json.search || []).slice(0, 5).forEach(entity => {
      ...
      const media = inferWikidataMedia(entity);
      out.overview.push({id: `wikidata-${entity.id}`, title: entity.label, aliases, media, description: entity.description || 'Wikidata候補', url: entity.concepturi, sourceName: 'Wikidata'});
    });
  }
  out.sources.push(sourceLink('Wikidata API', 'https://www.wikidata.org/w/api.php'));
  return out;
}
function inferWikidataMedia(entity) {
  const text = `${entity.label || ''} ${entity.description || ''}`.toLowerCase();
  if (/アニメ|anime|テレビ番組|television series|animated series/.test(text)) return 'アニメ';
  if (/漫画|manga|comic/.test(text)) return '漫画';
  if (/映画|film|movie/.test(text)) return '映画';
  if (/ドラマ|drama/.test(text)) return 'ドラマ';
  if (/小説|novel|book|書籍/.test(text)) return '小説';
  return '出典';
}
...
function renderResults(data) {
  ...
  $('#resultSummary').textContent = resultSummaryText(data);
  const visibleMedia = mediaSummary(data).filter(item => item.count).map(item => item.media);
  ...
  $('#resultsContent').innerHTML = [judgementSummarySection(data), ...visibleMedia.map(media => mediaSection(media, mediaDisplayItems(data, media), data.query, data)), maybeSection(data), relatedSection('類似作品', data.relatedCandidate.length ? data.relatedCandidate : data.similar), genreSection(data), confirmSection(data.query), sourceSection(data), searchDebugSection(data)].filter(Boolean).join('');
...
function countResults(data) { return mainCandidates(data).length; }
function relatedCount(data) { return uniqueBy([...(data.relatedCandidate || []), ...(data.similar || [])], duplicateKey).length; }
function resultSummaryText(data) {
  const total = countResults(data);
  const maybe = fuzzyCandidates(data).length;
  const related = relatedCount(data);
  return total ? `保存不要で閲覧可。候補 ${total}件。${maybe ? ` もしかして ${maybe}件。` : ''}` : `候補なし。${maybe ? `もしかして ${maybe}件。` : ''}${related ? `類似 ${related}件。` : ''}確認リンクあり。`;
}
...
function mediaSection(media, items, query, data) {
  ...
  const bestNote = items.length !== normalItems.length ? '<p class="duplicate-note">本命候補以外は、もしかして・類似作品へ分けています。</p>' : '';
  return `<section id="sec-${media}" class="panel"><div class="section-head"><h2>${media}</h2><span class="count">${normalItems.length}件</span></div>${bestNote}<div class="card-grid">${shown.map(item => itemCard(item, data)).join('')}</div>${restHtml}</section>`;
}
