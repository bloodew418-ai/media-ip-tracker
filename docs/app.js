...
function confirmLinksHtml(title, media = '') {
  const primary = primaryConfirmLinks(title);
  const secondary = secondaryConfirmLinks(title, media);
  return `<div class="confirm-links primary-links">${primary.map((link, index) => `<a class="link-button ${index === 0 ? 'primary' : ''}" href="${esc(link.url)}" target="_blank" rel="noopener">${esc(link.label)}</a>`).join('')}</div><details class="more-links"><summary>他のサービスで探す</summary><div class="confirm-links secondary-links">${secondary.map(link => `<a class="link-button" href="${esc(link.url)}" target="_blank" rel="noopener">${esc(link.label)}</a>`).join('')}</div></details>`;
}
...
function descriptionHtml(item, max = 180) {
...
function summaryChips(values, emptyText, className = '') {
  return values.length ? `<div class="summary-chips ${className}">${values.map(value => `<span class="chip">${esc(value)}</span>`).join('')}</div>` : `<p class="muted compact-text">${esc(emptyText)}</p>`;
}
...
function judgementSummarySection(data) {
  ...
  const foundMedia = mediaSummary(data).filter(item => item.count).map(item => `${item.media} ${item.count}件`);
  const missingMedia = mediaSummary(data).filter(item => !item.count).map(item => item.media);
  const savedGroup = currentResultGroupId ? saved.find(group => group.id === currentResultGroupId) : findGroupByTitle(data.query);
  const sources = data.sources.map(src => src.name).filter(Boolean).slice(0, 4).join(' / ') || '公開データ';
  return `<section id="sec-作品判定サマリー" class="panel judgement-summary"><div class="summary-header"><div><p class="eyebrow">作品判定サマリー</p><h2>${esc(data.query)}</h2></div><span class="count">候補 ${total}件</span></div><div class="summary-grid"><div class="summary-card keyword"><span>検索キーワード</span><strong>${esc(data.query)}</strong></div><div class="summary-card best"><span>最有力候補</span><strong>${best ? esc(best.title) : '候補なし'}</strong><small>${best ? esc([best.media, best.year].filter(Boolean).join(' / ')) : '別表記で確認できます'}</small></div><div class="summary-card"><span>見つかった媒体</span>${summaryChips(foundMedia, '候補なし', 'found-media')}</div><div class="summary-card subtle"><span>未確認媒体</span>${summaryChips(missingMedia, 'なし', 'missing-media')}</div><div class="summary-card compact"><span>候補数</span><strong>${total}件</strong><small>${esc(sources)}</small></div><div class="summary-card compact"><span>保存状態</span><strong>${savedGroup ? '保存済み' : '未保存'}</strong><small>${savedGroup ? `${savedGroup.items.length}件の媒体候補` : '保存なしで閲覧中'}</small></div></div>${mediaExpansionHtml(data)}${best ? bestCandidateHtml(best) : zeroResultHtml(data.query)}</section>`;
}
function bestCandidateHtml(item) { return `<div class="best-candidate"><div class="candidate-kicker">最有力候補</div><div class="best-candidate-head"><div><h3>${esc(item.title)}</h3><p class="muted">断定ではなく、候補の一致度から優先表示しています。</p></div><div class="meta-pills"><span class="chip">${esc(item.media)}</span>${item.year ? `<span class="chip">${esc(item.year)}</span>` : ''}</div></div>${descriptionHtml(item, 220)}${confirmLinksHtml(item.title, item.media)}<details class="source-details"><summary>データ出典を見る</summary><div class="source-links">${sourceLinksHtml(item) || '<span class="muted">データ出典リンクはありません。</span>'}</div></details></div>`; }
function mediaExpansionHtml(data) { return `<div class="media-expansion" aria-label="媒体展開まとめ">${mediaSummary(data).map(item => `<div class="media-pill ${item.count ? 'found' : 'empty'}"><strong>${esc(item.media)}</strong><span>${item.count ? `候補あり ${item.count}件` : '候補なし'}</span></div>`).join('')}</div>`; }
function zeroResultHtml(query) { return `<div class="zero-guidance"><div><p class="eyebrow">候補なし</p><h3>候補が見つかりませんでした</h3><p>失敗ではありません。別表記や日本向け確認リンクで探せます。</p></div><h4>別表記で探す</h4><div class="chips genre-links">${spellingSuggestions(query).map(value => `<button onclick="${scriptAttr(`runSearch(${JSON.stringify(value)})`)}">${esc(value)}</button>`).join('')}</div><h4>日本向け確認リンク</h4><div class="confirm-links primary-links">${primaryConfirmLinks(query).map((link, index) => `<a class="link-button ${index === 0 ? 'primary' : ''}" href="${esc(link.url)}" target="_blank" rel="noopener">${esc(link.label)}</a>`).join('')}</div><details class="more-links"><summary>他のサービスで探す</summary><div class="confirm-links secondary-links">${secondaryConfirmLinks(query, '映画').map(link => `<a class="link-button" href="${esc(link.url)}" target="_blank" rel="noopener">${esc(link.label)}</a>`).join('')}</div></details><button class="save-button subtle" type="button" onclick="document.querySelector('#manualDialog').showModal()">必要なら手動で補助保存</button></div>`; }
...
function itemCard(item, data) {
  ...
  return `<article class="card"><div class="card-title"><h3>${esc(normalized.title)}</h3><span class="chip">${esc(normalized.media)}</span></div>${dupes > 1 ? `<p class="duplicate-note">類似候補 ${dupes}件（同じタイトル・年・媒体）</p>` : ''}<dl class="candidate-meta"><div><dt>媒体種別</dt><dd>${esc(normalized.media)}</dd></div><div><dt>年</dt><dd>${esc(normalized.year || '不明')}</dd></div><div><dt>出典元</dt><dd>${esc(normalized.sourceName || '不明')}</dd></div></dl>${descriptionHtml(normalized, 150)}<div class="chips">${(normalized.genres || []).slice(0, 5).map(genre => `<span class="chip">${esc(genre)}</span>`).join('')}</div>${confirmLinksHtml(normalized.title, normalized.media)}<details class="source-details"><summary>データ出典を見る</summary><div class="source-links">${sourceLinksHtml(normalized) || '<span class="muted">データ出典リンクはありません。</span>'}</div></details><div class="save-area"><button class="save-button" onclick="${scriptAttr(`saveCandidate(${JSON.stringify(normalized)})`)}">候補を保存</button>${savedStatusHtml(normalized)}</div></article>`;
}
function emptyMedia(media, query) { return `<div class="empty"><strong>${esc(media)}は候補なし</strong><p>別表記や日本向けリンクで確認できます。</p><div class="confirm-links primary-links">${primaryConfirmLinks(query).map(link => `<a class="link-button" target="_blank" rel="noopener" href="${esc(link.url)}">${esc(link.label)}</a>`).join('')}</div></div>`; }
function relatedSection(title, items) { return `<section id="sec-${title}" class="panel"><h2>${title}</h2><div class="card-grid small">${items.map(item => `<article class="mini-card"><h3>${esc(item.title)}</h3><p>${esc(item.media)} / ${esc(item.reason)}</p><a href="${esc(item.url)}" target="_blank" rel="noopener">データ出典を見る</a></article>`).join('') || '<p class="muted">類似作品候補はまだありません。</p>'}</div></section>`; }
...
function sourceSection(data) { return `<section id="sec-出典" class="panel"><h2>データ出典</h2><div class="mini-list">${data.sources.map(src => `<div class="mini-item"><a target="_blank" rel="noopener" href="${esc(src.url)}">${esc(sourceLabel(src.name))}</a><small>${esc(src.checkedAt)}</small></div>`).join('')}</div></section>`; }
